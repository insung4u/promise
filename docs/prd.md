...
### 전투 화면 (Phaser Canvas)
- 모바일 세로형(Portrait)만 지원. 전체 해상도: **390×844** (iPhone 14 논리 해상도)
- Phaser 맵 영역: 390×480 (HUD 60px + 스킬버튼 80px + 스와이프존 224px 제외)
- 거점 3개: 상단 중앙(적), 맵 중앙(중립), 하단 중앙(아군 시작)
  - 적 거점: (195, 80)
  - 중립 거점: (195, 240)
  - 아군 거점: (195, 400)
- 하단: 4개 스킬 버튼 + 오토/수동 토글 (스킬 버튼 영역, ~80px)
- 최하단: 스와이프 명령 존 — 4방향 + 중앙 집결 (~224px)
- 최상단 HUD: 남은 시간 + 점수 + 전투 모드(공격/방어) 표시 (~60px)
