...
390×480 전투 맵을 Phaser로 구현한다. (전체 앱 해상도: 390×844 세로형)
...
전체 앱 구조 (390×844):
- HUD 영역: y=0~60
- Phaser 맵 영역: y=60~540 (높이 480)
- 스킬 버튼 영역: y=540~620 (높이 80)
- 스와이프 명령 존: y=620~844 (높이 224)
...
export class BattleScene extends Phaser.Scene {
  ...
  private drawBackground(): void {
    // 풀 (초록) — 전체 배경 (390×480)
    this.add.rectangle(195, 240, 390, 480, 0x4a7c59);
    // 중앙 길 (갈색) — 세로 방향 도로
    const road = this.add.graphics();
    road.fillStyle(0x8b7355);
    road.fillRect(160, 0, 70, 480);   // 세로 도로
    // 산 (회색) — 장애물 영역
    this.add.rectangle(60, 160, 80, 60, 0x6b6b6b);
    this.add.rectangle(310, 320, 80, 60, 0x6b6b6b);
  }

  private createCapturePoints(): void {
    const points = [
      { id: 0, x: 195, y:  80, owner: 'enemy'   as const },  // 맵 상단 중앙 (적 거점)
      { id: 1, x: 195, y: 240, owner: 'neutral' as const },  // 맵 중앙 (중립 거점)
      { id: 2, x: 195, y: 400, owner: 'player'  as const },  // 맵 하단 중앙 (아군 거점)
    ];
...
- [ ] `npm run dev` → 390×480 맵 렌더링 확인 (전체 앱 390×844 세로형)
